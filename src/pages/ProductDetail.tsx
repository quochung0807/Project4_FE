...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
      try {
        const response = await fetch(`https://electrostore-ofl1.onrender.com/api/products/slug/${slug}`, {
          headers: {
  ...
  useEffect(() => {
    if (product) {
      ...
      const fetchReviews = async () => {
        ...
        try {
          const res = await fetch(`https://electrostore-ofl1.onrender.com/api/reviews/product/${product.id}/`);
